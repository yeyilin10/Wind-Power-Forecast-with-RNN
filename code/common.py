...
def adjust_learning_rate(optimizer, epoch, args):
    ...
    Returns:
        None
    """
    # learning rate is adjusted that the learning rate declines as the epoch increases, 
    # allowing the loss function to decrease fast at the early stage of the training while avoiding divergence at the later
    # stage. 
...
class EarlyStopping(object):
    """
    Desc:
        EarlyStopping to aviod overfitting
    """
    ...
    def save_checkpoint(self, val_loss, model, path, tid, log_path):
        # type: (nn.MSELoss, Model, str, int, str) -> None
        """
        Desc:
...
class Experiment(object):
    ...
    def process_one_batch(self, model, batch_x, batch_y):
        # type: (Model, torch.tensor, torch.tensor) -> (torch.tensor, torch.tensor)
        """
        Desc:
            Process a batch
        Args:
    ...
    def inference_one_sample(self, model, sample_x):
        # type: (Model, torch.tensor) -> torch.tensor
        """
        Desc:
